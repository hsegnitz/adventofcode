...
class unit
{
    ...
    public function attack(&$grid)
    {
        ...
        /** @var unit $victim */
        $victim = $enemies[0];
        $resultingHP = $victim->registerHit(static::$attackPower);
...
class elf extends unit
{
    ...
    public function registerHit($power)
    {
        $res = parent::registerHit($power);
        if ($res <= 0) {
            throw new RuntimeException('An elf died, reset timeline!');
        }
        return $res;
    }
...
for ($power = 4; $power < 1000; $power++) {
    try {
        ...
        echo $e->getMessage(), " power: ", $power, " tick: ", $tick, "\n";
        continue;
    }

    echo "SO MUCH WIN -- power: ", $power;
    break;
}
