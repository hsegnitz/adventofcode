...
public class Part2 {
    ...
    public static void main(String[] args) {
        ...
        long firstLeft = 0;
        for (long y = 10; y < 10000; y++) {
            boolean newRow = true;
            for (long x = firstLeft; x < 10000; x++) {
                boolean isAffected = isAffected(y, x);
                ...
                if (isAffected(y+99, x)) {
                    System.out.println("HIT at y:" + y);;
                    System.out.println("HIT at x:" + x);;
    ...
    private static boolean isAffected(long y, long x) {
        IntCodeProgram program = new IntCodeProgram(input);
        program.addInput(x);
        program.addInput(y);

        return program.run() == 1L;
    }
