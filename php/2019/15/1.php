...
class Solver
{
    ...
    public function __construct() {
        ...
        $this->heap->insert(['program' => $program, 'x' => 0, 'y' => 0, 'distance' => 0, 'oxygen' => false]);
        $this->seen["0,0"] = 0;

        $oxyX = $oxyY = $maxX = $maxY = PHP_INT_MIN;
        $minX = $minY = PHP_INT_MAX;

        while ($this->heap->count() > 0) {
            $current = $this->heap->extract();

            foreach ($this->getNeighbours($current['program'], $current['x'], $current['y'], $current['distance']) as $next) {
                ...
                $coord = $next['x'] . "," .  $next['y'];
                if (isset($this->seen[$coord])) {
                    continue;
                }
                $this->heap->insert($next);
                $this->seen[$coord] = $next['distance'];

                if ($next['oxygen']) {
                    echo "part 1: ", $next['distance'], "\n";
                    $oxyX = $next['x'];
                    $oxyY = $next['y'];
                }
            }
        ...
        $changed = true;
        $minutes = 0;
        $this->seen[$oxyX . "," . $oxyY] = 'O';
        while ($changed) {
            $minutes++;
            $changed = $this->flood($minX, $minY, $maxX, $maxY);
        }

        echo "part 2: ", ($minutes - 1);
    }

    private function flood(int $minX, int $minY, int $maxX, int $maxY): bool
    {
        $changed = false;
        $newSeen = [];
        for ($x = $minX; $x <= $maxX; $x++) {
            for ($y = $minY; $y <= $maxY; $y++) {
                ...
                if ($this->neighbourIsOxy($x, $y)) {
                    $newSeen[$key] = 'O';
                    $changed = true;
                    continue;
                }
                $newSeen[$key] = $this->seen[$key];
            }
        }
        $this->seen = $newSeen;
        return $changed;
    ...
    private function neighbourIsOxy(int $x, int $y): bool
    {
        $keys = [
            ($x+1) . ',' . $y,
            ($x-1) . ',' . $y,
        ...
        foreach ($keys as $key) {
            if (isset($this->seen[$key]) && $this->seen[$key] === 'O') {
                return true;
            }
        }
    ...
    private function getNeighbours(\IntCodeProgram $program, int $x, int $y, int $distance): array
    {
        $neighbours = [];

        foreach (self::MOVES as $code => $move) {
            $neighbourProgram = clone $program;
            $neighbourProgram->addInput($code);
            switch ($neighbourProgram->run()) {
                case 1:
                    $neighbours[] = ['program' => $neighbourProgram, 'x' => ($x + $move[0]), 'y' => ($y + $move[1]), 'distance' => $distance + 1, 'oxygen' => false];
                    break;
                case 2:
                    $neighbours[] = ['program' => $neighbourProgram, 'x' => ($x + $move[0]), 'y' => ($y + $move[1]), 'distance' => $distance + 1, 'oxygen' => true];
                    break;
                case 0:
                default:
                    break;
