...
#0$lines = file('example.txt', FILE_IGNORE_NEW_LINES);
#$lines = file('example1.txt', FILE_IGNORE_NEW_LINES);
...
class Wanderer {
    ...
    public function getScoreOf(int $startRow, int $startCol): int {
        $arrayKeyHashMap = new ArrayKeyCache();
        foreach ($this->findListOfNines($startRow, $startCol, 1) as $coordinates) {
            $arrayKeyHashMap->store($coordinates, true);
        }
        return count($arrayKeyHashMap);
    }
...
foreach ($startPoints as $startPoint) {
    $score += $wanderer->getScoreOf($startPoint[0], $startPoint[1]);
}
